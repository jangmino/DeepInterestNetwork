...
class Model(object):
  ...
  def __init__(self, user_count, item_count, cate_count, cate_list):
    ...
    self.u = tf.placeholder(tf.int32, [None,], name="pholder_u") # [B]
    self.i = tf.placeholder(tf.int32, [None,], name="pholder_i") # [B]
    self.j = tf.placeholder(tf.int32, [None,], name="pholder_j") # [B]
    self.y = tf.placeholder(tf.float32, [None,], name="pholder_y") # [B]
    self.hist_i = tf.placeholder(tf.int32, [None, None], name="pholder_i") # [B, T]
    self.sl = tf.placeholder(tf.int32, [None,], name="pholder_sl") # [B]
    self.lr = tf.placeholder(tf.float64, [], name="pholder_ls")
    ...
    # wide part
    u_end_dim = tf.shape(u_emb)[1]-1
    i_end_dim = tf.shape(i_emb)[1]-1
    d_layer_wide_i = tf.concat([tf.gather(u_emb, [0], axis=-1) * tf.gather(i_emb, [0], axis=-1), tf.gather(u_emb, [u_end_dim], axis=-1) * tf.gather(i_emb, [i_end_dim], axis=-1),
    ...
    d_layer_wide_j = tf.concat([tf.gather(u_emb, [0], axis=-1) * tf.gather(j_emb, [0], axis=-1), tf.gather(u_emb, [u_end_dim], axis=-1) * tf.gather(j_emb, [i_end_dim], axis=-1),
